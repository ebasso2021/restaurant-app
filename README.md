...
| 2 · Cocina e inventario | Stock con **Alerta** y **Cantidad máxima**, botón **Agregar item**, cuadro y botón **+ Stock** (suma a lo existente), orden de compra automática, predicción de demanda (día y hora) con ajuste por IA, registro de merma. Stock en **g, kg, mL, L o unidades**; el costo es **siempre por kg, por L o por unidad** con **2 decimales** (y se muestra por g/mL). **Valor del stock = stock convertido a kg/L × costo** (4.500 g a CA$8,50/kg = CA$38,25); **costo promedio ponderado** en cada compra; **📒 Kardex** de cada compra, venta (cuenta cobrada), merma y ajuste, con saldo y costo promedio, exportable a CSV (en la web se guarda en la pestaña **Kardex** de la hoja de Google). |
...
| 2 · Kitchen & inventory | Stock with **Alert** and **Maximum quantity**, **Add item** button, **+ Stock** box and button (adds to existing), automatic purchase order, demand forecast (day and hour) with AI adjustment, waste log. Stock in **g, kg, mL, L or pieces**; the cost is **always per kg, per L or per piece** with **2 decimals** (also shown per g/mL). **Stock value = stock converted to kg/L × cost** (4,500 g at CA$8.50/kg = CA$38.25); **weighted average cost** on each purchase; **📒 Kardex** of every purchase, sale (paid bill), waste and adjustment, with balance and average cost, CSV export (on the website it is saved in the **Kardex** tab of the Google Sheet). |
