...
| 2 · Kitchen & inventory | Stock with **Alert** and **Maximum quantity**, **Add item** button, **+ Stock** box and button (adds to existing), automatic purchase order, demand forecast (day and hour) with AI adjustment, waste log. Units in **kg / L / pieces**, cost with **2 decimals** per kg/L and cost per **g/mL**; **weighted average cost** on each purchase; **📒 Kardex** of every purchase, sale (paid bill), waste and adjustment, with balance and average cost, CSV export (on the website it is saved in the **Kardex** tab of the Google Sheet). |
